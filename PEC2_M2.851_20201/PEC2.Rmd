...
```{r}
shapiro.test(df$Parch)
```

## Aplicación de pruebas estadísticas para comparar los grupos de datos. En función  de los datos y el objetivo del estudio, aplicar pruebas de contraste de hipótesis,  correlaciones, regresiones, etc. Aplicar al menos tres métodos de análisis  diferentes. 

Podemos ver que existe una correlación positiva alta entre "Sexo" y "Survived". De igual modo los predictores "Pclass" y "Fare" contribuyen a determinar "Survived" y están correlacionados negativamente entre sí, lo cual tiene sentido puesto que un precio elevado corresponde con una categoría de camarote superior (Pclass = 1 es mejor que Pclass = 2) y éstos estarían situados en la parte superior de la embarcación. 


```{r}
#Convertimos las variables de tipo factor a numeric para poder estudiar la correlación entre todas ellas
...
corrplot.mixed(corr = cor(df[,c("SibSp", "Parch", "Sex", "Age", "Fare", "Pclass", "Embarked", "Survived")],
                          method = "pearson"))
```
...
```{r}
#Convertimos Survived a factor para obtener labels que sean "0" o "1"
df$Survived <- as.factor(df$Survived)
modelo.logit <- glm(data = df, formula = Survived ~ Sex + Age + Pclass + Fare, family = "binomial")
summary(modelo.logit)
```

Se observa que un incremento de una unidad en Sex, es decir, cuando el sexo es femenino (ajustando por las otras regresoras) aumenta las odds de
sobrevivir en 13.81, que un incremento de una unidad en Age (ajustando por las otras regresoras) reduce las odds
de sobrevivir en 0.96, que un incremento de una unidad en Pclass (ajustando por las otras regresoras)
reduce las odds de sobrevivir en 0.29, y que un incremento de una unidad en Fare (ajustando por las otras regresoras)
reduce las odds de sobrevivir en 0.99

Vemos que el mayor impacto en incrementar la probabilidad de supervivencia es el regresor "Sex", mientras que en sentido contrario es "PClass"
...
```{r}
confusionMatrix(table(predict(modelo.logit, type="response") >= 0.5,df$Survived != 1))
```
